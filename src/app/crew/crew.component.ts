...
export class CrewComponent implements OnInit {
  ...
  addCrewMember(candidate: object) {
    ...
    if (this.crew.includes(candidate)) {
      // console.log('crew is included')
      this.inCrew = true;
    }
    else {
      // console.log('crew member is not included')
      this.inCrew = false
    }
    if (this.crew.length < 3 && this.inCrew === false) {
      // console.log('adding crew member')
      this.crew.push(candidate);
      // console.log(this.crew)
    } else if (this.inCrew === true) {
      // console.log(this.crew.indexOf(candidate))
      this.crew.splice(this.crew.indexOf(candidate), 1)
      // console.log('crew is removed')
    }
  }
}
